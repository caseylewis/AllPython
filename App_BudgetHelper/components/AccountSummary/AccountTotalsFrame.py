...
class AccountTotalsFrame(StandardFrame):
    def __init__(self, root):
        ...
        # YEARLY
        self._total_yearly_lbl = StandardLabel(self, text="Yearly", **StandardLabel.left_style_args)
        self._total_yearly_lbl.grid(row=1, column=0, **StandardLabel.grid_args)

        self._total_yearly_display = StandardLabel(self, "", **StandardLabel.right_style_args)
        self._total_yearly_display.grid(row=1, column=1, **StandardLabel.grid_args)

        # MONTHLY
        self._total_monthly_lbl = StandardLabel(self, text="Monthly", **StandardLabel.left_style_args)
        self._total_monthly_lbl.grid(row=2, column=0, **StandardLabel.grid_args)

        self._total_monthly_display = StandardLabel(self, "", **StandardLabel.right_style_args)
        self._total_monthly_display.grid(row=2, column=1, **StandardLabel.grid_args)

        # SEMI MONTHLY
        self._total_semi_monthly_lbl = StandardLabel(self, text="Semi-Monthly", **StandardLabel.left_style_args)
        self._total_semi_monthly_lbl.grid(row=3, column=0, **StandardLabel.grid_args)

        self._total_semi_monthly_display = StandardLabel(self, "", **StandardLabel.right_style_args)
    ...
    def show_account_stats(self, expenses_list):
        ...
        self._total_yearly_display.set_decimal(total_yearly)

        total_monthly = total_yearly / 12
        self._total_monthly_display.set_decimal(total_monthly)

        total_semi_monthly = total_monthly / 2
        self._total_semi_monthly_display.set_decimal(total_semi_monthly)

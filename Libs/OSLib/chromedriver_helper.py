...
def download_chrome_driver(dir):
    ...
    # GET DESKTOP PATH TO ADD THE TEMP DIR TO
    desktop_path = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')

    # TEMP DIRECTORY STARTS WITH DESKTOP PATH
    temp_dir = r'{}\tempdownload'.format(dir)
    ...
    chrome_driver_zip_download_link = 'https://chromedriver.storage.googleapis.com/{}/chromedriver_win32.zip'.format(
        latest_version)

    # CREATE TEMP DIR AND CHANGE TO IT
    if not os.path.exists(temp_dir):
        os.mkdir(temp_dir, mode=666)
    os.chdir(temp_dir)
    ...
    write_file = open(zip_file_name, 'wb')
    write_file.write(r.content)

    # EXTRACT ZIP FILE
    zip_location = r"{}\{}".format(temp_dir, zip_file_name)
    ...
    chromedriver_exe_location = r"{}\{}".format(temp_dir, chrome_driver_exe_name)
    app_chromedriver_exe_location = r"{}\{}".format(dir, chrome_driver_exe_name)

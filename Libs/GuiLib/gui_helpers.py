...
#         super().__init__(root)  # ARGS AND KWARGS WILL BE ADDED LATER VIA .CONFIG()
#         self.grid_columnconfigure(0, weight=1)
#         self.grid_rowconfigure(0, weight=1)
#
#         # CREATE KEY CHILD WIDGETS
#         self._canvas = Canvas(self, bd=0, bg="white", highlightthickness=1)
#         self._canvas.grid_columnconfigure(0, weight=1)
#         self.view_port = Frame(self._canvas, bg="black")
#         self._v_scrollbar = Scrollbar(self, orient=VERTICAL, command=self._canvas.yview)
#
...
#         self._canvas_window = self._canvas.create_window((4, 4), window=self.view_port, anchor="nw", tags="self.viewPort")
#
#         # SET EVENTS FOR VIEWPORT AND CANVAS RESIZING
#         self.view_port.bind("<Configure>", self.__on_frame_configure)
#         self._canvas.bind("<Configure>", self.__on_canvas_configure)
#
...
#
#         # CONFIGURE ATTRIBUTES THAT ARE GIVEN ON INIT NOW THAT ALL WIDGETS ARE CREATED
...
#         self._canvas.configure(yscrollcommand=self._v_scrollbar.set)  # attach scrollbar action to scroll of canvas
#
...
#         self._canvas.itemconfig(self._canvas_window, width=canvas_width, height=canvas_height)
#         self.view_port.config(width=canvas_width, height=canvas_height)
#         # self.__on_frame_configure(None)
#
...
#             # self.viewPort.bind_all("<Button-4>", self.onMouseWheel)
#             # self.viewPort.bind_all("<Button-5>", self.onMouseWheel)
#         else:
...
class ContentFrame(ScrollFrame):
    def __init__(self, root, title, *args, **kwargs):
        super().__init__(root, hide_scroll_bar=False, *args, **kwargs)
        self.title = title
